...
namespace slamware_ros_sdk {
    ...
    {
        if (0.005f < msg->linear.x)
            pltfm.moveBy(rpos::core::FORWARD);
        else if (msg->linear.x < -0.005f)
            pltfm.moveBy(rpos::core::BACKWARD);

        if (0.001f < msg->angular.z)
            pltfm.moveBy(rpos::core::TURNLEFT);
        else if (msg->angular.z < -0.001f)
            pltfm.moveBy(rpos::core::TURNRIGHT);
    }
